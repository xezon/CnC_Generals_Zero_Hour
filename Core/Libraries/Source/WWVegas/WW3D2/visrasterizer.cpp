...
struct EdgeStruct
{
	...
	{
		Y = WWMath::Ceilf_Origin(verts[top].Y);
		Height = WWMath::Ceilf_Origin(verts[bottom].Y) - Y;
...
int IDBufferClass::Render_Occluder_Scanline(GradientsStruct & grads,EdgeStruct * left,EdgeStruct * right)
{
	...
	int xstart = WWMath::Float_To_Long(WWMath::Max(WWMath::Ceilf_Origin(left->X),1.0f));
	int width = WWMath::Float_To_Long(WWMath::Ceilf_Origin(right->X)) - xstart;
...
int IDBufferClass::Render_Non_Occluder_Scanline(GradientsStruct & grads,EdgeStruct * left,EdgeStruct * right)
{
	...
	int xstart = WWMath::Float_To_Long(WWMath::Max(WWMath::Ceilf_Origin(left->X),1));
	int width = WWMath::Float_To_Long(WWMath::Ceilf_Origin(right->X)) - xstart;
	if (xstart + width > ResWidth) {
		width = ResWidth - xstart;
	}
